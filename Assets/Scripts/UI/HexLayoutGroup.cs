...
namespace MPewsey.HexagonalUI
{
    ...
    public class HexLayoutGroup : LayoutGroup
    {
        ...
        [SerializeField]
        private CellPlacement _nextCellPlacement;
        /// <summary>
        /// Custom property, set where the next cell is placed.
        /// Works only with Horizontal startAxis & Horizontal CellOrientation settings for now
        /// </summary>
        public CellPlacement NextCellPlacement
        {
            get => _nextCellPlacement;
            set => SetProperty(ref _nextCellPlacement, value);
        }
        ...
        private void CalculateLayout()
        {
            ...
            for (int i = 0; i < rectChildren.Count; i++)
            {
                ...
                if (CellOrientation == Axis.Horizontal)
                {
                    ...
                    if (IsEven(column))
                    {
                        if(NextCellPlacement is CellPlacement.OnTop)
                            y += 0.5f * (cellSize.y + Spacing.y);
                        
                        else if(NextCellPlacement is CellPlacement.OnBottom)
                            y -= 0.5f * (cellSize.y + Spacing.y);
                    }
                }
                else
        ...
        /// <summary>
        /// Set each Hexagon name based on their peculiar component
        /// </summary>
        /// <param name="i">Index of the child</param>
        /// <param name="row">Row position of the hexagon</param>
        /// <param name="column">Column position of the hexagon</param>
        private void SetChildrenName(int i, int row, int column)
        {
            string nameAttribute;
            if (rectChildren[i].GetComponent<HexagonPowered_Active>())
            {
                nameAttribute = "Active";
            }
            else if (rectChildren[i].GetComponent<HexagonDirectional>())
            {
                nameAttribute = "Directional";
            }
            else if (rectChildren[i].GetComponent<HexagonTarget>())
            {
                nameAttribute = "Target";
            }
            else
            {
                nameAttribute = "Empty";
            ...
            rectChildren[i].name = $"Hex_{nameAttribute} ({row}, {column})";
        }
